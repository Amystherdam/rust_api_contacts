  ...
  S: Send + Sync, // Exemplo de restrição para S
{
  ...
  async fn from_request(req: Request<B>, _state: &S) -> Result<Self, Self::Rejection> {
    let Json(request_contact) = req
      .extract::<Json<RequestTask>, _>() // Provide both generic arguments
      .await
...
pub async fn create_contact(
  ...
  Json(request_contact): Json<RequestTask>
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
  let new_task = contacts::ActiveModel {
  ...
  let result = new_task.save(&database).await.map_err(|e| {
    (
